...
fit_bycatch <- function(formula, data, time = "year", effort = "effort",
  ...
  datalist = list(n_row = nrow(data),
    ...
    K = ncol(X),
    x = X,
    family = ifelse(family=="poisson",0,1),
    time_varying = as.numeric(time_varying))

  mod <- rstan::sampling(object=stanmodels$bycatch,
    data = datalist,
    pars = pars,
    iter = iter,
    chains = chains,
  ...
  return(list("data" = data, "effort"=effort, "events"=names(mf), "time"=time, "fitted_model"=mod))
}
