...
plot_pred_distribution <- function(fit, year, xlab = "Predicted Bycatch", ylab = "Frequency", breaks = NULL) {

  exp <- bycatch::get_total(fit)

  model_years <- fit$data$Year
  ...
  h <- hist(
    predicted,
    main = paste("Distribution of Predicted Bycatch -", year),
    xlab = xlab,
    ylab = ylab,
    breaks = breaks,
    xlim = c(0, max(predicted))
  )
  text(
    x = h$mids,
    y = h$counts,
